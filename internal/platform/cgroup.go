...
// GetCgroupInterfacePath returns base path for current process' cgroup interface files.
func GetCgroupInterfacePath(options ...Option) (string, error) {
	cfg := &config{}
	...
	if cfg.ProcfsPath == "" {
		cfg.ProcfsPath = "/proc/self"
	}

	mount, err := mountPointFromFile(filepath.Join(cfg.ProcfsPath, "mountinfo"))
	if err != nil {
		return "", fmt.Errorf("platform(cgroup): failed to get cgroup2 mountpoint: %w", err)
	}

	name, err := nameFromFile(filepath.Join(cfg.ProcfsPath, "cgroup"))
	if err != nil {
		return "", fmt.Errorf("platform(cgroup): failed to get cgroup name: %w", err)
	}

	return filepath.Join(mount, name), nil
}

// getCPUQuotaFromCgroup returns cgroup cpu quota for the current process.
func getCPUQuotaFromCgroup(options ...Option) (float64, error) {
	cfg := &config{}
	...
	if cfg.CgroupInterfacePath == "" {
		path, err := GetCgroupInterfacePath(options...)
		if err != nil {
			return 0, err
		}
		cfg.CgroupInterfacePath = path
	}

	// Read cpu.max
	v, err := getCPUQuotaFromFile(filepath.Join(cfg.CgroupInterfacePath, "cpu.max"))
	if err != nil {
		return 0, fmt.Errorf("platform(cgroup): failed to get cpu quota: %w", err)
	}
	return v, nil
}

// getMemoryQuotaFromCgroup returns cgroup cpu quota for the current process.
//
//nolint:nonamedreturns // for docs.
func getMemoryQuotaFromCgroup(options ...Option) (max, high int64, err error) {
	cfg := &config{}
	...
	// Read memory.max
	max, err = memLimitFromFile(filepath.Join(cfg.CgroupInterfacePath, "memory.max"))
	if err != nil {
		return 0, 0, fmt.Errorf("platform(cgroup): failed to get memory max: %w", err)
	}

	// Read memory.high
	high, err = memLimitFromFile(filepath.Join(cfg.CgroupInterfacePath, "memory.high"))
	if err != nil {
		return 0, 0, fmt.Errorf("platform(cgroup): failed to get memory high: %w", err)
	}
