...
func getInfo(mountInfo string, cgroup string) (*Info, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cgroup: failed to get cgroup mountpoint: %w", err)
	}

	name, err := getCgroupFromFile(cgroup)
	if err != nil {
		return nil, fmt.Errorf("cgroup: failed to get cgroup name: %w", err)
	}
...
// getCgroupFromFile returns cgroup name for given cgroup file.
func getCgroupFromFile(path string) (string, error) {
	if path == "" {
		path = defaultCgroupNamePath
	}
...
//
// This function unescapes the above sequences.
